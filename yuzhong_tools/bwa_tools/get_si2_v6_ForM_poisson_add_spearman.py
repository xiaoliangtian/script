...
class HMM:
    ...
    def viterbi(self):
        ...
        P_Tmin = round( delta[T-1, :].min(), 20)
        I[T-1] = delta[T-1, :].argmin()
        for t in range(T-2, -1, -1):
            I[t] = psi[ t+1, I[t+1] ]
        #I2[T-1] = delta[T-1, :].argmin()
        #diff = (P_T - 0)*math.pow(10, 300)/((P_T+P_Tmin)*math.pow(10, 300))
        #for t in range(T-2, -1, -1):
        #    I2[t] = psi[ t+1, I[t+1] ]
        print(P_T,P_Tmin,I)
        return(I, P_T) 
        ...
        for line in lines:
            ...
            if (line[0:5] != "chr11" or (line[0:5] == "chr11" and (int(items[1]) < 5144348 or int(items[1]) > 5347511) )) and items[0] !="#CHROM" and int(items[dad].split(":")[-2])>80 and int(items[sonCol].split(":")[-2]) > 80:
                ...
                if Fgt== Sgt == "0/0":
                    ...
                    Pgt = "1/1"
                    bbbbbb.append(pos)
                elif Fgt == "0/0" and Sgt == "1/1":
                    Pgt = "0/1"
                    aabbab.append(pos)
                elif Fgt == "1/1" and Sgt == "0/0":
                    Pgt = "0/1"
                    bbaaab.append(pos)
                else:
                    Pgt = ''
...
###creat spearman correction group
def get_Spear():
    global H0,H1,y
    H0,H1 = [],[]
    y = []
...
E, e ,num_homo = get_Ee()
H0,H1, y = get_Spear()
rH0, pvalueH0 = stats.stats.spearmanr(H0,y)
rH1, pvalueH1 = stats.stats.spearmanr(H1,y)
t,pt = stats.ttest_1samp(H0, y)
#print(t, pt)
print(rH0, pvalueH0,rH1, pvalueH1)
B = get_B(Si)
Pi = [0.5, 0.5]
hmm = HMM(A, B, Pi, Si)
I, P_T = hmm.viterbi()
