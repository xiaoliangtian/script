...
for i in `cat list`; do 
 { read -u6
   { sleep 1
     ...
     perl /home/tianxl/pipeline/STR_SNPlinkage/STRfq2needfq_Pool2_v3.pl $i*R1.fastq.gz $i*R2.fastq.gz all "$i"_R1.fastq "$i"_R2.fastq $i > analyse/$i.R1.fastq
     mkdir SNP
     cd analyse
     ...
     perl /home/tianxl/pipeline/STR_SNPlinkage/abstract_need_fastqSTR.pl $i.uniq.R1.fastq $i.need.list > $i.map.fastq 
     ...
     perl /home/tianxl/pipeline/STR_SNPlinkage/STR_SNP.statPool2.pl $i.sam  $i.type 2>$i.log
