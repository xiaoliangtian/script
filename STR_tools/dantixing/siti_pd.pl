...
#my $header = <IN>;
#$header =~s/\n//;
$header = "chr\tpos\tpos\tdad_type\tpos\tmom_type\tpos\tson_type\tSTR\tdad_type\tmom_type\tson_type\tMMMM\tMMMP\tMMPP\tPPPP\tPPPM\n";
print "$header";
while(<IN>) {
 chomp;
 $line=$_;
...
if(exists $hash3{MMPP} or exists $hash3{MPMP} or exists $hash3{MPPM}  or exists $hash3{PMMP} or exists $hash3{PMPM} or exists $hash3{PPMM} ) {
	print "YES\t";
	$hash5{$chr.'_'.MMPP}++;
}
if(!exists $hash3{MMPP} and !exists $hash3{MPMP} and !exists $hash3{MPPM}  and !exists $hash3{PMMP} and !exists $hash3{PMPM} and !exists $hash3{PPMM} ) {
	print "NO\t";
}
