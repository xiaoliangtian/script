...
for i in `cat list`; do 
 { read -u6
   { sleep 1
     ...
     perl /home/tianxl/pipeline/STR_SNPlinkage/STRfq2needfq_Pool1_v3.pl $i*R1.fastq.gz $i*R2.fastq.gz all "$i"_R1.fastq "$i"_R2.fastq $i > analyse/$i.R1.fastq
     mkdir SNP
     cd analyse
     ...
     bowtie2 -x /home/tianxl/database/hg19/hs37d5/v2/hs37d5.fasta $i.map.fastq -k 1 -p 20 |  samtools sort -@ 20 | samtools view - > $i.sam
     perl /home/tianxl/pipeline/STR_SNPlinkage/STR_SNP.statPool1.pl $i.sam  $i.type 2>$i.log
