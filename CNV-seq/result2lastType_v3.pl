...
my $header = <IN>;
print OUT "cnv\tchromosome\tstart\tend\tsize\ttype\ttrue_type\tband\tchromosome.start.end\tVariant\t区带起\t区带止\t变异分型\t类型\t分子核型起点\t分子核型止点\t大小\tOMIM基因\tConrad.Common.CNV\tDGV.GoldStandard.Common.CNV\t1000Genomes.Common.CNV\tDECIPHER.Common.CNV\tHaploinsufficiency(LOD)\tExAC.genes.constraints\tExAC.pLI\tdecipher_urls\tgene_num\tgene_name\tomim_num\tgene_pheno\n";
while (<IN>) {
    chomp;
    my @line = split( /\t/, $_ );
    $cnv  = $line[0];
    ...
    $out2 = join("\t",@line[9..12]);
    print OUT "$cnv\t$chromosome\t$start\t$end\t$size\t$type\t$true_type\t$band\t$chromosome_start_end\t$CNV_type\t$band_start\t$band_end\t$cnv_type\t$cnv_type1\t$type_start\t$type_end\t$type_size\t$omim_num\t$out1\t$out2\n";
}
