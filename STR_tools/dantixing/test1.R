...
plot.test <- function(data, chrom.gap=2e7, colour=5, title=NA, ylim=c(-2,2), xlabel='Chromosome', ylabel="copies")
{
    ...
    xbreaks <- c(xbreaks,0,259350000,522550000,740550000,951650000,1152650000,1343750000,1522950000,1689350000,1850550000,2006150000,2161150000,2324500000,2459700000,2587500000,2700150000,2810450000,2911650000,3009750000,3088950000,3176600000,3248100000,3311450000,3486750000,3576000000)
    ybreaks <- c(ybreaks,0,1,2,3,4)
	data <- temp
	data$color <- as.character(data$color)
    cbPalette <- c("blue", "red", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")
	p <- ggplot()+geom_point(data=data, map=aes(x=gpos, y=log2, colour=data$color), size = 0.6) + geom_vline(xintercept=xbreaks,colour='lightgrey',size=0.8) + geom_hline(yintercept=ybreaks,colour='lightgrey',size=0.8,linetype='dotted')+labs(title="全基因组检测结果", x='', y=ylabel)
	p <- p + scale_y_continuous(ylabel, lim=ylim, breaks=seq(0, 4, 1))
