            ...
            if($hashresult{$start1.'_'.$next1} >10 or $hashresult{$start1.'_'.$next2} > 10 ) {
                if($hashresult{$start1.'_'.$next1} >10 and $hashresult{$start1.'_'.$next1} > $hashresult{$start1.'_'.$next2}) {
                    ...
                    if($start1[1] =~ /\-/) {
                        $start1[1] =~ s/\-//;
                        #print "$start1\n";
                        $hashcontigs{$next1} = substr($hashcontigs{$next1},length($start1[1])-1,length($hashcontigs{$next1})-(length($start1[1])-1));
                        print "$start1\t$hashcontigs{$next1}\n";
                    }
                    if($start2[1] =~ /\-/) {
                        $start2[1] =~ s/\-//;
                        $hashcontigs{$next2} = substr($hashcontigs{$next2},length($start2[1])-1,length($hashcontigs{$next2})-(length($start2[1])-1));
                    }
                    ...
                    if($start1[1] =~ /\-/) {
                        $start1[1] =~ s/\-//;
                        $hashcontigs{$next2} = substr($hashcontigs{$next2},length($start1[1])-1,length($hashcontigs{$next2})-(length($start1[1])-1));
                        print "$start1\t$hashcontigs{$next2}\n";
                    }
                    if($start2[1] =~ /\-/) {
                        $start2[1] =~ s/\-//;
                        $hashcontigs{$next1} = substr($hashcontigs{$next1},length($start2[1])-1,length($hashcontigs{$next1})-(length($start2[1])-1));
                    }
...
if($start1[1] =~ /\-/) {
    $start1[1] =~ s/\-//;
    $contigDel = $contig;
    $contigDel = substr($contigDel,length($start1[1])-1,length($contigDel)-(length($start1[1])-1));
    $lastread1 = $combContig1.$contigDel;
}
else {
...
if($start2[1] =~ /\-/) {
    $start2[1] =~ s/\-//;
    $contigDel = $contig;
    $contigDel = substr($contigDel,length($start2[1])-1,length($contigDel)-(length($start2[1])-1));
    $lastread2 = $combContig2.$contigDel;
}
else {
