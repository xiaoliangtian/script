...
my $dad_var3; 
my $header = <IN>;
print "pos\tdad_genotype\tdad_frequency\tmom_genotype\tmom_frequency\tson_genotype\tson_frequency\tson_from_dm\tson_from_dm%\tsum_dm\tson_from_dad\tson_from_dad%\tsum_dad\tson_from_mom\tson_from_mom%\tsum_mom\tson_off_dm\tson_off_dm\tsum_off_dm\n";
while(<IN>) {
 chomp;
 $line=$_;
...
if($sum_son > 100 and $sum_mom >= 100 and $sum_dad > 100) {
     foreach $dad_type(keys %hashson) {
		 ...
		 $sum_son_from_dm1 += $hashson1{$dad_type[1]};
		 }
		 else {
		...
		if ($dad_type[1] =~ /[0-9]\(/ ){
		 ...
		 shift @sum_type;
                 $dad_type2 = join("\+",@sum_type);
                 push @son_from_dad,$dad_type2;
                 push @son_from_dad1,$hashson1{$dad_type[1]};
		 $sum_son_from_dad1 += $hashson1{$dad_type[1]};
                 }
                 else {
		...
		if ($dad_type[1] =~ /[0-9]\(/ ){
		 ...
		 shift @sum_type;
                 $dad_type2 = join("\+",@sum_type);
                 push @son_from_mom,$dad_type2;
                 push @son_from_mom1,$hashson1{$dad_type[1]};
		 $sum_son_from_mom1 += $hashson1{$dad_type[1]};
                 }
                 else {
		...
		$sum_son_from_mom1 += $hashson1{$dad_type[1]};
		 ...
		 shift @sum_type;
                 $dad_type2 = join("\+",@sum_type);
                 push @son_off_dm,$dad_type2;
                 push @son_off_dm1,$hashson1{$dad_type[1]};
		 $sum_son_off_dm1 += $hashson1{$dad_type[1]};
                 }
                 else {
		...
		$son_var1 =~s/\([A-Z]+\)//g;
		$dad_var1 =~s/\([A-Z]+\)//g;
		print "$len{$line[0]}\t$dad_var1\t$dad_var2\t$mom_var1\t$mom_var2\t$son_var1\t$son_var2\t$son_from_dm\t$son_from_dm1\t$sum_son_from_dm1\t$son_from_dad\t$son_from_dad1\t$sum_son_from_dad1\t$son_from_mom\t$son_from_mom1\t$sum_son_from_mom1\t$son_off_dm\t$son_off_dm1\t$sum_son_off_dm1\n";
