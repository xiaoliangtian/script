...
foreach $chr1 ( sort { ( split /D/, $a )[1] <=> ( split /D/, $b )[1] } keys %hash4 ) {
	...
	foreach $y(@chr_result) {
		@chr_result1 = split(/\t/,$y);
		#print "$chr_2\t$chr_result1[0]\n";
		if($chr_2 eq $chr_result1[0]) {
			#print "$chr_2\t$chr_result1[0]\n";
			@line_hapy = split /\t/, $y;
			$line_hapy[2] = abs($line_hapy[2]-$band{$chr_2});
			if ($rate12 >= $rate10 and (split /\//,$line_hapy[3])[0] ne (split /\//,$line_hapy[3])[1]) {
				$hash_me{$line_hapy[2]} = $line_hapy[12].'_'.$line_hapy[13];
			}
			if ($rate12 < $rate10 and (split /\//,$line_hapy[4])[0] ne (split /\//,$line_hapy[4])[1]) {
				$hash_me{$line_hapy[2]} = $line_hapy[8].'_'.$line_hapy[9];
			}
		}
	}
		
	foreach (sort { $a <=> $b} keys %hash_me) {
		$hash_me_num++;
		#print "$hash_me_num\n";
		if ($hash_me_num == 1) {
			@hash_me = split(/\_/,$hash_me{$_});
			if($hash_me[0] eq "YES") {
				$me_type1 = m1m1;
			}
			elsif($hash_me[1] eq "YES") {
				$me_type1 = m1m2;
			}
			else {
				$me_type1 = un;
