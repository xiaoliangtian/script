...
inputData = '/home/tianxl/pipeline/STR_tools/STR_ku/new/all.erti.rate' 
headIdx = []
fatherIdx = {}
sonIdx = {}
momIdx = {}
...
calNegOut = []
calOtherOut = []
cal_script = '/home/tianxl/pipeline/STR_tools/STR_ku/misa_result_Y.pl'
...
def loop_cal(fcol, mcol, scol, head=False):
    cmd = ['perl', cal_script, '-i', inputFile, '-o', 'test',  '-f', fcol, '-m', mcol, '-s', scol]
    if not head:
        cmd.append('-noheader')
...
with open(inputFile) as textFile:
    ...
    for head in heads:
        ...
        if len(sample) > 1 and sample[0][:1] == 'F' and idx > 0:
            headIdx.append(idx)
            if 'F-' in sample[0]:
                sample_fix = sample[0][2:]
            else:
                sample_fix = sample[0][1:]
            if 'S' in sample_fix:
...
reportOut = '\n'.join(calOut)
varReport = fileDir + '/' + 'var-report_chrY.txt'
