...
calNegOut = []
calOtherOut = []
cal_script = '/home/tianxl/pipeline/PGD_STR/STR_sameOrNot_v4.pl'
...
def loop_cal(mcol, scol, head=False):
    cmd = ['perl', cal_script, '-i', inputFile,  '-o', 'test',  '-m', mcol, '-s', scol]
    #if not head:
        #cmd.append('-noheader')
...
for i in sonIdx.keys():
    ...
    for h in sonIdx.keys():
        if i != h:
                #if i[:4] == j[:4] and i[:4]==k[:4] and i[:4]==h[:4]:
            famOut = loop_cal(sonIdx[i], sonIdx[h])
            print (famOut)
            isPos(famOut, h)
    #for j in son2Idx.keys():
        #if i != j:
        #    famOut = loop_cal(son2Idx[i], son2Idx[j])
        #    print (famOut)
        #    isPos(famOut, j)
