...
my $sumRatio = sprintf("%.2f",$count/$normal);
my $effectRatio = sprintf("%.2f", $effect / $count);
if($sumRatio <0.66) {
    $sumResult = "($sumRatio)数据量偏少";
}
else {
    $sumResult = "($sumRatio)正常";
}
if($effectRatio <0.6) {
    $effectRatio .= "有效数据比例偏低";
}
print ADAPT "$sample\t$adapt\t$count\t"
  . $adapt / $count . "\t"
  . $effectRatio .$sumResult ."\n";
