...
while(<IN>) {
 ...
 if(substr($line,0,2)ne"##") {
  ...
  if($line_count>1 and substr($line[$mom],0,3)ne"./." and substr($line[$son],0,3)ne"./." and substr($line[$dad],0,3)ne"./." and $mom[2]ne"." and $son[2]ne"." and $dad[2]ne"." and $mom[2]>=1000 and $son[2]>=1000 and $dad[2]>=1000) {
  #my @line=split/\t/,$line;
 # print $line."\n";
#  my @mom=split/\:/,$line[$mom];  
  my @mom_var=split/\,/,$mom[1];
...
#  print $line."\n";
  ...
  if(($line[3]ne"-" and $line[4] ne "-" and length($line[1])<=1 and length($line[2])<=1) and ($mom[0]eq "0/0" or $mom[0]eq "1/1") and ($dad[0] eq "0/1")and ($dad_var >0.25 and $dad_var <0.75) ) {  
