...
#my $mom_var=$mom_var[1]/$mom[2];
#  my @son=split/\:/,$line[$son];
        $line_count1 = $line_count - 1;
        if ( $line_count > 1 && $line[-2] eq '-' ) {
            my @son_var = split /\,/, $son[1];
            ...
            if ( $son_var[0] > $son_var[1] ) {
                ...
                if ( $dad[1] =~ ',' ) {
                    ...
                    print OUT "$line_count1" . "\t" . 'a' . "\t" . $sim1 . "\n";
                }
                else {
                    $sim2 = 1 - $son_small;
                    print OUT "$line_count1" . "\t" . 'a' . "\t" . $sim2 . "\n";
                }
            }
        }
        elsif ( $line_count > 1 && $line[-2] ne '-' ) {
            my @son_var = split /\,/, $son[1];
            if ( $son_var[0] < $son_var[1] ) {
                my $son_small = $son_var[0] / $son[2];
                $sim3 = 0 - $son_small;
                print OUT "$line_count1" . "\t" . 'b' . "\t" . $sim3 . "\n";
            }
            if ( $son_var[0] > $son_var[1] ) {
                my $son_small = $son_var[1] / $son[2];
                $sim4 = 0 - $son_small;
                print OUT "$line_count1" . "\t" . 'b' . "\t" . $sim4 . "\n";
            }
        }
    }
}
