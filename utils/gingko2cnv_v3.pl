...
$list=0;
my $head1 = <IN2>;
print CNV "chromosome\tstart\tend\ttest\tref\tposition\tlog2\tyline\tp.value\tcnv\tcnv.size\tcnv.log2\tcnv.p.value\tcolor\n";
print CNV2 "chromosome\tstart\tend\ttest\tref\tposition\tlog2\tyline\tp.value\tcnv\tcnv.size\tcnv.log2\tcnv.p.value\tcolor\n";
while ( defined( my $v1 = <IN3> ) and defined( my $v2 = <IN2> ) ) {
    chomp( $v1, $v2 );
    my @line1 = split( /\t/, $v1 );
    my @line2 = split( /\t/, $v2 );
    ...
    $hash_chr{ $line1[0] }++;
    if ( $line2[1] >= 2 ) {
        print CNV2 "$chr\t$line1[1]\t$line1[2]\t100\t100\t$pos\t$line2[1]\t$line1[$dam]\t0\t0\t0\t0\t0\tblue\n";
    }
    if ( $line2[1] < 2 ) {
        print CNV2 "$chr\t$line1[1]\t$line1[2]\t100\t100\t$pos\t$line2[1]\t$line1[$dam]\t0\t0\t0\t0\t0\tred\n";
    }
    if ( !exists $hash6{ $line1[0] . '_' . $line1[1] } and $line2[1] >= 2 and ( $project eq 'CNV' or $project eq 'NIPT' ) ) {
        print CNV "$chr\t$line1[1]\t$line1[2]\t100\t100\t$pos\t$line2[1]\t$line1[$dam]\t0\t0\t0\t0\t0\tblue\n";
    }
    if ( !exists $hash6{ $line1[0] . '_' . $line1[1] } and $line2[1] >= 2 and $project eq 'SC' ) {
        $hap = $line2[1];
        $hap = log( $hap / 2 ) / log(2);
        print CNV "$chr\t$line1[1]\t$line1[2]\t100\t100\t$pos\t$hap\t$line1[$dam]\t0\t0\t0\t0\t0\tblue\n";
    }
    if ( !exists $hash6{ $line1[0] . '_' . $line1[1] } and $line2[1] < 2 and ( $project eq 'CNV' or $project eq 'NIPT' ) ) {
        print CNV "$chr\t$line1[1]\t$line1[2]\t100\t100\t$pos\t$line2[1]\t$line1[$dam]\t0\t0\t0\t0\t0\tred\n";
    }
    if ( !exists $hash6{ $line1[0] . '_' . $line1[1] } and $line2[1] > 0.25 and $line2[1] < 2 and $project eq 'SC' ) {
        $hap = $line2[1];
        $hap = log( $hap / 2 ) / log(2);
        print CNV "$chr\t$line1[1]\t$line1[2]\t100\t100\t$pos\t$hap\t$line1[$dam]\t0\t0\t0\t0\t0\tred\n";
    }
    if ( !exists $hash6{ $line1[0] . '_' . $line1[1] } and $line2[1] < 0.5 and $line2[1] > 0 and $project eq 'SC' ) {
        print CNV "$chr\t$line1[1]\t$line1[2]\t100\t100\t$pos\t-2\t$line1[$dam]\t0\t0\t0\t0\t0\tred\n";
    }
...
foreach ( sort {(split/chr/,$a)[1] <=> (split/chr/,$b)[1]} keys %hash1 ) {
...
if ( $project eq 'SC' or $project eq 'CNV') {
    `Rscript $DIR/SC.R $ARGV[4] $ARGV[4] `;
    `sed '/^X/d' $ARGV[4] > $ARGV[4].noXY`;
    `sed -i '/^Y/d' $ARGV[4].noXY`;
    `Rscript $DIR/SC.R $ARGV[4].noXY $ARGV[4].noXY`;
}
...
foreach $h ( sort { $a <=> $b } keys %hash4 ) {
    foreach $i ( sort { ( split /\_/, $a )[1] <=> ( split /\_/, $b )[1] } keys %hash5 ) {
		...
		#print CNV1 "$sum\n";
            ...
            elsif ( ( $num > 1 and ( ( split /\_/, $i )[1] - $end ) > 1 ) or ( $num > 1 and $hash3{$i} != $type ) ) {
                $cnv_num++;
                $num++;
                $lens      = $end - $start;
                $true_type = $sum / $num1;
