  	...
  	if($line_count>1 and $line[0]!~ 'DX'and $line[0]!~ 'DY' and $line[$mom] ne 'NA' and $line[$mom] ne "" and $line[$son] ne 'NA' and $line[$son] ne "" and $line[$dad] ne 'NA' and $line[$dad] ne "") {
		...
		if($sum_son > 100 and $sum_mom >100 and $sum_dad > 50) {
			...
			foreach $dad_type(keys %hashdad) {
                                $dad_type_rate= sprintf("%.2f",$hashdad{$dad_type}/$sum_dad);
			...
			foreach $mom_typen(keys %hashmom) {
                                $mom_type_raten= sprintf("%.2f",$hashmom{$mom_typen}/$sum_mom);
                                if ($mom_type_raten>0.15) {
                                        ...
                                        if(length($STR_s1)> 0) {
                                                ...
                                                $type_lastn = $son_from_mom_typen.'.'.length($STR_s1)%4;
                                        }
                                        else {
			...
			if( $line[0]!~'DX' and $line[0]!~'DY' and $line[$mom] ne 'NA' and $line[$dad] ne 'NA' and $line[$son] ne 'NA' and $line[$son] ne "") {  
         			foreach $STR_s(keys %hashson){
         				...
         				if ($mom_rate < 0.01 and ($son_rate-$mom_rate)>$rate and ($son_rate-$mom_rate) < 0.2 and $hashson{$STR_s}>$min and $sum_mom_type_rate >0.7 and $sum_dad_type_rate > 0.7 and $num_type <=2 and $num_type > 0 and exists $type{$line[0].'_'.$STR_s} ) {
						...
						#}
							...
							push @line_str,$line1;
							#print OUT2 "$line[0]\t$dad_type_last\t$mom_type_last\t".$type_last.'/'."$type_from_mom_last\t-\n";
							$hash_son_num_type{$line1} = $type_last.'/'.$type_from_mom_last;
							#print OUT2 "$line[0]\t".$type_last.'/'."$type_from_mom_last\n";   
          					}
          					elsif($dad_rate< 0.1 and abs($type_last-$type_from_mom_last)!=1 and abs($type_last-$type_off_mom_last)!=1 and !exists $len{$line[0].'_'.$STR_s}) {
					...
					if ($line_str >1 ) {
						...
						if($zhichi>0) {
							...
							#print "$line[0]\t$hash_zong{$line[0].'_'.((split/\_/,$line_str_zhichi))[0]}\n";
							print OUT2 "$line[0]\t$dad_type_last\t$mom_type_last\t$hash_son_num_type{$line_str_zhichi}\t-\n".
							push @result,$line_str_zhichi;
						}
						else {
							$zong+=$hash_zong{$line[0].'_'.(split/\_/,$line_str_cuopei)[0]};
							#print "$hash_zong{$line[0].'_'.(split/\_/,$line_str_cuopei)[0]}\n";
							print OUT2 "$line[0]\t$dad_type_last\t$mom_type_last\t$hash_son_num_type{$line_str_cuopei}\t错配\n".
						...
						if($line_str[0] !~ '错配'){
							$snp_f++;
							#print "$line[0]\t$hash_zong{$line[0].'_'.(split/\_/,$line_str[0])[0]}\n";
							print OUT2 "$line[0]\t$dad_type_last\t$mom_type_last\t".$type_last.'/'."$type_from_mom_last\t-\n";
							push @result,@line_str;
						}
						else {
							#print "$hash_zong{$line[0].'_'.(split/\_/,$line_str[0])[0]}\n";
							print OUT2 "$line[0]\t$dad_type_last\t$mom_type_last\t".$type_last.'/'."$type_from_mom_last\t错配\n";
							push @result,@line_str;
						}
					}
       				@line_str = "";
