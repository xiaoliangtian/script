...
for i in `cat list`; do 
 { read -u6
   { sleep 1
	h=${i//./-}
	cnvkit.py batch $i.dedup.sorted.bam -r  /home/tianxl/pipeline/CNV-seq/GDCNV-seq/my_reference_100K.cnn -d analyse/ -p 10 
	wait 
	cd analyse/
	perl /home/tianxl/pipeline/CNV-seq/cnvkit2type_v2.pl $i.dedup.sorted.cns $i.type $i.qianhe > $i.cnv
        ...
        python /home/tianxl/pipeline/CNV_analyse/../utils/cnv-annotated_v2.py  $i.all.loci > "$i".all.intervals.result
        ...
        python /home/tianxl/pipeline/CNV_analyse/../utils/cnv-annotated_v2.py  $i.loci > "$i".intervals.result
