...
if($sum_son > 500 and $sum_mom >400 and $sum_dad > 300) {
	$all++;
     foreach $mom_type(keys %hashmom) {
		...
		if ( $son_type[1] <0.05  or  $son_type[0] <0.05) {
		print "false\n";
		}           
             }        
  if( $line[0]!~'DX' and $line[0]!~'DY' and $line[$mom] ne 'NA' and $line[$dad] ne 'NA' and $line[$son] ne 'NA' and $line[$son] ne "") {  
         foreach $STR_s(keys %hashson){
               ...
               if(exists $hashdad{$STR_s}) {
                 ...
                 #$num++;
         if (($mom_rate < 0.01 and ($son_rate-$mom_rate)>$rate and ($son_rate-$mom_rate) < 0.2 and $hashson{$STR_s}>$min and exists $type{$line[0].'_'.$STR_s}) ) {
...
my $youli=$ratio*2;
#print $zong."\n";
$ave_dad = $hashsum_dad/284;
$ave_mom = $hashsum_mom/284;
$ave_son = $hashsum_son/284;
