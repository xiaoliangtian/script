...
foreach my $file (@files) {
    ...
    if (-e $file2) {
        ...
        `java -jar $DIR/../third-party/trimmomatic-0.32.jar PE -threads 30 $file1 $file2 clean/"$sample"_R1.fastq.gz $sample.rm_1.fq clean/"$sample"_R2.fastq.gz $sample.rm_2.fq ILLUMINACLIP:$adaptor:2:30:10 LEADING:20 TRAILING:20 SLIDINGWINDOW:4:20 MINLEN:20  TOPHRED33`;
    }else {
        `java -jar $DIR/../third-party/trimmomatic-0.32.jar SE -threads 30 $file1 clean/"$sample"_R1.fastq.gz ILLUMINACLIP:$adaptor:2:30:10 LEADING:20 TRAILING:20 SLIDINGWINDOW:4:13 MINLEN:$minlen TOPHRED33`;
    }
    `rm $sample.rm_*.fq`;
