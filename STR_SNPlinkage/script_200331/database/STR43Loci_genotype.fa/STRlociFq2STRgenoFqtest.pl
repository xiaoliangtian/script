...
while (<fastq>) {
    ...
    if ( $i == 4 && $match{ $loci } ) {
        ...
        if ( $seq =~ m/$match{$loci}/i ) {
		    ...
		    print "${$h}\t";
                    ...
                    if ( $motif ne $motif_old ) {
                        ...
                        $repeat1 = int( length( ${$h} ) / $motifLen{ $loci } );
                        $repeat2 = length( ${$h} ) % $motifLen{ $loci };
	    ...
	    print "$strseq\n";
            ...
            if ( exists $genoFa{ $loci . '_' . $strseq } ) {
                print "@" . "$solexatag" . "_" . "$loci" . "_" . "$genoFa{$loci.'_'.$strseq}\n$seq\n$symbol\n$qual\n";
            }
            else {
                print "@" . "$solexatag" . "_" . "$loci" . "_" . "$repeatSum" . '_' . "$strout\n$seq\n$symbol\n$qual\n";
...
foreach ( keys %refData ) {
    if ( exists $hashLoci{$_} and $hashLoci{$_} ne 0) {
        ...
        @sortGeno = sort { $hashNewGeno{$_}{$b} <=> $hashNewGeno{$_}{$a} } keys %{ $hashNewGeno{$_} };
        $num1     = $hashNewGenoNum{ $_ . '_' . $sortGeno[0] };
        $num2     = $hashNewGenoNum{ $_ . '_' . $sortGeno[1] };
        $num3     = $hashNewGenoNum{ $_ . '_' . $sortGeno[2] };
        $Ratio1   = sprintf( "%.2f", $num1 / $hashLoci{$_} );
        $Ratio2   = sprintf( "%.2f", $num2 / $hashLoci{$_} );
        $Ratio3   = sprintf( "%.2f", $num3 / $hashLoci{$_} );
        print NEWGENO "$_\t$hashLoci{$_}\t$hashNewGenoLoci{$_}\t$num1($Ratio1)\t$num2($Ratio2)\t$num3($Ratio3)\t$hashNewGenoNum{$_}{$sortGeno[0]}\n";
    }
    else {
        $hashNewGenoLoci{$_} = 0;
        $hashLoci{$_}        = 0;
        $newGenoRatio        = 0;
        print LOG "$_\t$hashLoci{$_}\t$hashNewGenoLoci{$_}($newGenoRatio)\t0(0)\t0(0)\t0(0)\tnone\n";
    }
}
close(NEWGENO);
