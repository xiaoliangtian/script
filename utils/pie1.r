...
ratio<-sprintf("%.2f",100*x[,1]/sum(x[,1]))
...
if (length(x[,1])>length(cols)) {
    ...
    cols<-c(cols,colorRampPalette(brewer.pal(8,"Dark2"))(length(x[,1])-length(cols)))
}
pie(x[,1],col=cols,labels=ratio,cex=.8)
legend("right",legend=x[,2],bty="n",inset=-0.5,fill=cols,cex=1.1)
dev.off()
