...
plot.test <- function(data, chrom.gap=2e7, colour=5, title=NA, ylim=c(-2,2), xlabel='Chromosome', ylabel="copies")
{
	...
	p <- ggplot()+geom_point(data=data, map=aes(x=gpos, y=log2, colour=data$color), size = 0.6) + geom_vline(xintercept=xbreaks,colour='lightgrey',size=0.8) + geom_hline(yintercept=ybreaks,colour='lightgrey',size=0.8,linetype='dotted')+labs(title=args[3], x='', y=ylabel)
