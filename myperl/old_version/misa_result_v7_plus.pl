		...
		if($sum_son > 100 and $sum_mom >100 and $sum_dad > 50) {
			...
			if( $line[0]!~'DX' and $line[0]!~'DY' and $line[$mom] ne 'NA' and $line[$dad] ne 'NA' and $line[$son] ne 'NA' and $line[$son] ne "") {  
         			foreach $STR_s(keys %hashson){
         				...
         				if ($mom_rate < 0.01 and ($son_rate-$mom_rate)>$rate and ($son_rate-$mom_rate) < 0.4 and $hashson{$STR_s}>$min and $sum_mom_type_rate >0.7 and $sum_dad_type_rate > 0.7 and $num_type <=2 and $num_type > 0 ) {
