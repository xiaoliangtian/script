...
def get_si(famvcf):
    ...
    global abaaaa, abaaba, abbbab, abbbbb, aaabaa, aaabab, bbabba, bbabbb, ababaa, ababbb,ababab,ababba
    aaaaaa, bbbbbb, aabbab, bbaaab = [], [], [], []
    abaaaa, abaaba, abbbab, abbbbb, aaabaa, aaabab, bbabba, bbabbb, ababaa, ababbb, ababab, ababba  = [], [], [], [], [], [], [], [], [], [], [], []
    ...
    with open(inputFile,"r") as fr:
        ...
        for line in lines:
            ...
            pos = items[1]
            if items[0] !="#CHROM" and items[dad].split(":")[0] != './.' and items[sonCol].split(":")[0] != './.'  and items[dad].split(":")[0] != './.' and items[sonCol].split(":")[0] != './.'   and items[mom].split(":")[0] != './.' and int(items[dad].split(":")[2]) > 0 and int(items[mom].split(":")[2]) >0  and int(items[sonCol].split(":")[2]) > 0:
            ...
            if (items[0] != "chr11" or (items[0] == "chr11" and (int(items[1]) < 5124816  or int(items[1]) > 5317312) )) and items[0] !="#CHROM" and items[dad].split(":")[0] != './.' and items[sonCol].split(":")[0] != './.' and int(items[dad].split(":")[2])>20 and int(items[sonCol].split(":")[2]) > 20:
                #print (pos)
                Fgt = items[dad].split(":")[0]
                Fgtd = items[dad].split(":")[2]
                Fvar = items[dad].split(":")[1].split(",")
                ...
                if Fgt== Mgt == "0/0":
                    ...
                    bbaaab.append(pos)
                if Mgt == "0/1" and Pgt == "0/1" and int(Mgtd) > 20  and Sratio > 0.4 and Sratio < 0.6 and Mratio > 0.4 and Mratio < 0.6:
                    MratioSum += Mratio
                    MratioPoint += 1
                    SratioSum += Sratio
                    SratioPoint += 1
            ...
            elif items[0] !="#CHROM" and items[dad].split(":")[0] != './.' and items[sonCol].split(":")[0] != './.'   and items[dad].split(":")[0] != './.' and items[sonCol].split(":")[0] != './.'   and items[mom].split(":")[0] != './.' and items[dad].split(":")[2] != '.' and items[sonCol].split(":")[2] != '.' and int(items[dad].split(":")[2])>20 and int(items[sonCol].split(":")[2]) > 80:
                pos = items[1] 
                if items[3] == ".":
                    aaaaaa.append(pos)
    ...
    ll = [aaaaaa, bbbbbb, aabbab, bbaaab, abaaaa, abaaba, abbbab, abbbbb, aaabaa, aaabab, bbabba, bbabbb, ababaa, ababbb,ababab,ababba]
    for i in range(len(ll)):
        #print(len(ll[i]))
        ll[i] = list( set( ll[i] ).intersection( set( hq_pos ) ) )
        #print(len(ll[i]))
    [aaaaaa, bbbbbb, aabbab, bbaaab, abaaaa, abaaba, abbbab, abbbbb, aaabaa, aaabab, bbabba, bbabbb,ababaa, ababbb,ababab,ababba] = ll
    ##pos for Si
    pos_hmm = []
    for i in [abaaaa, abaaba, abbbab, abbbbb, aaabaa, aaabab, bbabba, bbabbb,ababaa,ababbb,ababab,ababba]:
        #print(len(i))
