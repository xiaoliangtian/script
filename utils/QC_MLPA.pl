...
my $adaptor="$DIR/adapter_mlpa.fa";
my ($keep,$fastQC)=("","");
my $output;
GetOptions(
    "a:s" => \$adaptor,
...
foreach my $file (@files) {
    ...
    if (-e $file2) {
        ...
        `java -jar $DIR/../third-party/trimmomatic-0.32.jar PE -threads 30 $file1 $file2 $sample.clean_1.fq $sample.rm_1.fq $sample.clean_2.fq $sample.rm_2.fq ILLUMINACLIP:$adaptor:2:30:10 LEADING:20 TRAILING:20 SLIDINGWINDOW:4:12 MINLEN:50 CROP:77  TOPHRED33`;
    }else {
        `java -jar $DIR/../third-party/trimmomatic-0.32.jar SE -threads 30 $file1 $sample.clean_1.fq ILLUMINACLIP:$adaptor:2:30:10 LEADING:20 TRAILING:20 SLIDINGWINDOW:4:13 MINLEN:$minlen TOPHRED33`;
    }
    #`rm $sample.rm*_*.fq`;
    $hash{0}{$sample}=$sample;
    $plot{0}{$sample}=$sample;
    $hash{1}{$sample}=`zcat $file|wc -l `;
...
USAGE:
    ...
    -a ADAPTER     = ADAPTER file.Default = $DIR/adapter_mlpa.fa
